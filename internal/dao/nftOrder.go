...
func (d *Dao) CreateNftOrder(txn *GormDB, ce *model.ChainEvent) error {
	...
	e := model.NftOrder{
		...
		ItemId:       itemId,
		Value:        decimal.RequireFromString(util.ToString(value)).Div(decimal.RequireFromString("1000000000000")),
		Price:        decimal.RequireFromString(util.ToString(price)).Div(decimal.RequireFromString("1000000000000")),
